!     Read PAR satellite data (W/m**2) from GCIP/SRB little-endian 
...
!     of GCIP/SRB dat grid cell centers ---
!
      do lat = 1,nlat
      ...
      enddo
        ...
        do j = 1,ny
          ...
          do  i = 1,nx
           ...
           yg(i,j) = alat
!
! --- Check coordinates of modeling cell centers
!     Trap out of bounds coordinates to edge of GCIP/SRB grid ---
!
...
!
! --- Find the GCIP/SRB data to interpolate ---
!
        do 200 j = 1,ny
          do 200 i = 1,nx
            do l = 1,nlon
...
!
! --- Adjust for time zones used in GCIP/SRB data ---
!
            ...
            n2 = n + iadj2
!
! --- Skip early/late hours where a time zone
!     adjustment spills the GCIP/SRB data arrays ---
!
...
!
 ...
 200    continue
!
! --- Write gridded PAR to netcdf format file
!     replacing -999 codes with zero ---
!
        do i = 1,nx
          do j = 1,ny
...
!-----Interpolate hourly averages to instantaneous rad values

      do i=1,nx
        do j=1,ny

!-----Interpolate rad at 0h and 24h using trends
...
!-----
!  check for missing data (valid data exists for hr before and after)
